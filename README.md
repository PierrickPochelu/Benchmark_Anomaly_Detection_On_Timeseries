...
- either they contain too early anormaly which make it impossible to calibrate the algorithms on anomaly-free signals
- some of these files contains NaN values. I ignored them.

A few files are fully anomaly-free (e.g., realAWSCloudwatch/ec2_cpu_utilization_c6585a.csv). It is impossible to compute a F1-score for arithmetic reason (division per zero). To compare anomaly-detectors on them I computed the accuracy on it and put the accuracy in the F1 score-column.

# Let's compare a simple strategy on all datasets. It consists to used the normed signal frame-per-frame to IsolationForest. The length of frames is 128 values and 15% of signals is used as unsupervised training ("unsupervised" but we know there is no anomaly in it).
...
# Compare different FE algorithms by fixing the previous steps and the anomaly detector step (Isolation Forest)
"""
from offline_AD import OFFLINE_AD
...
"""
# We can see Auto-encoders is the best features extractors while ROCKET fails (and yet I tuned hyperparameters).
...
# "AE" means we extract features with AE (compression) and detect anomalies with another AE based on loss error. This cascade is way better than 1 single AE!
